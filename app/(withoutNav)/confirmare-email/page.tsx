...
// Confirmarea legării contului anonim de un email real. Link-ul din email
// poate fi deschis din orice browser (fără sesiune) — identitatea o dă tokenul.
const ConfirmareEmailContent = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/account/link-email/confirm", {
      method: "POST",
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center flex-col font-Unbounded">
      ...
      <div className="bg-transparent p-3 md:p-6 rounded-lg flex items-center justify-center flex-col max-w-[400px]">
        {status === "loading" && (
          <h1 className="text-mygray text-2xl md:text-3xl text-center">Se confirmă...</h1>
        )}
...
const ConfirmareEmail = () => (
  <Suspense>
    <ConfirmareEmailContent />
  </Suspense>
);
